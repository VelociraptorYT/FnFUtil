...
public class GunItem extends Item {
    ...
    public void releaseUsing(ItemStack stack, Level level, LivingEntity entity, int timeLeft) {
        ...
        Vec3 end = start.add(look.scale(500)); // 5 block range
        ...
        for (var target : entities) {
            ...
            if (toTarget.normalize().dot(look) > 0.98) {
                target.hurt(net.minecraft.world.damagesource.DamageSource.playerAttack(player), 2F);
                if (fnfmain.DEBUGGING) System.out.println("TG-Debug: Hit entity = " + target.getName().getString());
                break;
            }
        }

        level.playSound(null, player.getX(), player.getY(), player.getZ(), ModSounds.GUN_FIRE.get(), SoundSource.PLAYERS, 4.0F, 1.0F);
